...
private:
   ...
   void countActive_(TreeNode* root,int &count)const;
   void removeAll(TreeNode* const &temp);
   bool removeOne(string name);
   void removeOne_(TreeNode* const &prev_node, TreeNode* const &node_del,int if_left);
   TreeNode* findNode(string name);
   TreeNode* max(TreeNode* curNode);
   void insert_(TreeNode* new_node,TreeNode* cur_node);
   TreeNode* find_(TreeNode* root, string name);
